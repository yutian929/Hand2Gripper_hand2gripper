...
class Hand2GripperModel(nn.Module):
    ...
    def __init__(self, d_model: int = 256, img_size: int = 256,
        ...
        self.encoder = HandNodeEncoder(
            in_dim=25, hidden=d_model, out_dim=d_model,
    ...
    @staticmethod
    def _build_node_features(kp_xyz_norm: torch.Tensor,
                              is_right: torch.Tensor) -> torch.Tensor:
        """拼接 [xyz | onehot | is_right] -> [B,21,25]"""
        ...
        feats = torch.cat([kp_xyz_norm, onehot, isr], dim=-1)
        return feats
    ...
    def forward(self, img_crop: torch.Tensor, keypoints_3d: torch.Tensor,
        ...
        node_feats = self._build_node_features(kp_xyz_norm, is_right)
